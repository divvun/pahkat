...
#[no_mangle]
extern "C" fn pahkat_config_path(handle: *const MacOSPackageStore) -> *const c_char {
...
#[no_mangle]
extern "C" fn pahkat_config_repos(handle: *const MacOSPackageStore) -> *const c_char {
    let store = safe_handle!(handle);
...
#[no_mangle]
extern "C" fn pahkat_config_set_repos(handle: *const MacOSPackageStore, repos: *const c_char) {
    let store = safe_handle!(handle);
...
#[no_mangle]
extern "C" fn pahkat_config_set_cache_path(
    handle: *const MacOSPackageStore,
    ...
    store
        .config()
        .set_cache_base_path(std::path::PathBuf::from(&*cache_path))
        .unwrap();
}

#[no_mangle]
extern "C" fn pahkat_config_cache_path(handle: *const MacOSPackageStore) -> *const c_char {
    let store = safe_handle!(handle);
    CString::new(&*store.config().cache_base_path().to_string_lossy())
        .unwrap()
        .into_raw()
}

#[no_mangle]
extern "C" fn pahkat_client_free(handle: *const MacOSPackageStore) {
    if handle.is_null() {
        return;
    }
...
extern "C" fn pahkat_str_free(handle: *mut c_char) {
    ...
    unsafe {
        CString::from_raw(handle);
    }
}

#[no_mangle]
extern "C" fn pahkat_repos_json(handle: *const MacOSPackageStore) -> *const c_char {
    let store = safe_handle!(handle);
...
#[no_mangle]
extern "C" fn pahkat_refresh_repos(handle: *const MacOSPackageStore) {
    let store = safe_handle!(handle);
    store.refresh_repos();
}

#[no_mangle]
extern "C" fn pahkat_force_refresh_repos(handle: *const MacOSPackageStore) {
    let store = safe_handle!(handle);
